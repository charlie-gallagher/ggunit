...
## Usage
Central to the ggunit package is the design unit "lines." One line is the 
...
The `device_dimensions()` function is useful because it's explicit. If you don't
have very strict standards, you can pass your graphic to `ggunit_save()`, which
is a thin wrapper around `ggplot2::ggsave()`. 
...
which is the number of points in 1mm. To make this easier to use, `ggunit` 
provides the `pt_to_mm()` and `mm_to_pt()` functions. 

```{r}
default_text <- 11      # Text size in points

default_text / .pt      # Converted to millimeters
